...
if ($alreadyListening -match 'YES') {
  ...
  Invoke-RouterCommand -Command ": > /opt/var/log/xkeen-manager-uhttpd.log && cd $RemoteRoot && /opt/sbin/uhttpd -f -p 0.0.0.0:$Port -h $RemoteRoot -I index.html -x /api -i .cgi=/bin/sh -r 'Opengater' >/opt/var/log/xkeen-manager-uhttpd.log 2>&1 &"
}
