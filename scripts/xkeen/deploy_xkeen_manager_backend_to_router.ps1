param(
  ...
  [string]$RemoteSelfhealInit = "/opt/etc/init.d/S25opengater-selfheal",
  [string]$RemoteSingboxInit = "/opt/etc/init.d/S24opengater-singbox",
  [string]$RemoteSysctlInit = "/opt/etc/init.d/S20opengater-sysctl",
  [string]$RemoteInitScript = "/opt/etc/init.d/S26opengater",
  [string]$RemoteCronScript = "/opt/etc/cron.1min/50-opengater-selfheal",
  [string]$RemoteFsHook = "/opt/etc/ndm/fs.d/50-opengater.sh",
  [string]$RemoteUsbHook = "/opt/etc/ndm/usb.d/50-opengater.sh"
)
...
$localSelfhealLoop = Join-Path $repoRoot 'scripts\xkeen\opengater-selfheal-loop.sh'
$localSelfhealInit = Join-Path $repoRoot 'scripts\xkeen\opengater-selfheal.initd.sh'
$localSingboxInit = Join-Path $repoRoot 'scripts\xkeen\opengater-singbox.initd.sh'
$localSysctlInit = Join-Path $repoRoot 'scripts\xkeen\opengater-sysctl.initd.sh'
$localInitScript = Join-Path $repoRoot 'scripts\xkeen\opengater.initd.sh'
$localCronScript = Join-Path $repoRoot 'scripts\xkeen\opengater-selfheal.cron.sh'
$localRemountHook = Join-Path $repoRoot 'scripts\xkeen\opengater-remount-hook.sh'
...
if ! command -v sing-box >/dev/null 2>&1; then
  ...
  rm -rf /tmp/opengater-sing-box /tmp/opengater-sing-box.tar.gz
  mkdir -p /tmp/opengater-sing-box
  if wget --no-check-certificate -O /tmp/opengater-sing-box.tar.gz "$SING_BOX_URL" >/dev/null 2>&1; then
    tar -xzf /tmp/opengater-sing-box.tar.gz -C /tmp/opengater-sing-box
    SING_BOX_BIN="$(find /tmp/opengater-sing-box -type f -name sing-box | head -n 1)"
...
if [ -f "$XRAY_INIT" ] && grep -q 'ARGS="run -confdir /opt/etc/xray"' "$XRAY_INIT" 2>/dev/null && ! grep -q 'ARGS="run -confdir /opt/etc/xray/configs"' "$XRAY_INIT" 2>/dev/null; then
  cp "$XRAY_INIT" "$XRAY_INIT.bak-opengater-confdir" 2>/dev/null || true
