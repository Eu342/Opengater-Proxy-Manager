#!/bin/sh
# Запуск установщика Opengater Proxy Manager через Keenetic Web CLI.
#
# Использование в http://192.168.1.1/a после установки Entware:
#
#   exec sh -c "/opt/bin/curl -fsSL https://raw.githubusercontent.com/Eu342/Opengater-Proxy-Manager/main/scripts/xkeen/opm-web-cli-install.sh | /opt/bin/sh"
#
# Если curl ещё не установлен в Entware:
#
#   exec sh -c "/opt/bin/opkg install curl >/dev/null 2>&1; /opt/bin/curl -fsSL https://raw.githubusercontent.com/Eu342/Opengater-Proxy-Manager/main/scripts/xkeen/opm-web-cli-install.sh | /opt/bin/sh"
#
# Этот скрипт не ставит Entware. Он только запускает обычный on-router
# установщик Opengater Proxy Manager из Web CLI без отдельного SSH-подключения.
...
log() {
  printf '==> %s\n' "$*"
  logger -t opm-web-cli "$*" 2>/dev/null || true
}

die() {
  printf 'ОШИБКА: %s\n' "$*" >&2
  logger -t opm-web-cli "ОШИБКА: $*" 2>/dev/null || true
  exit 1
}
...
main() {
  ...
  log "Скачиваем установщик Opengater Proxy Manager (через $FETCHER_TYPE)"
  fetch_to "$INSTALL_URL" "$INSTALL_FILE" || die "не удалось скачать $INSTALL_URL"
  chmod 700 "$INSTALL_FILE" 2>/dev/null || true

  log "Запускаем установщик Opengater Proxy Manager"
