...
start_ui() {
  ...
  if is_running; then
    echo "opm already listening on $PORT"
  ...
  /opt/sbin/uhttpd -f -p 0.0.0.0:$PORT -h "$ROOT_DIR" -I index.html -x /api -i .cgi=/bin/sh -r 'Opengater Proxy Manager' >>"$LOG_FILE" 2>&1 &
  sleep 2
...
case "$1" in
  ...
  status)
    if is_running; then
      echo "opm running on $PORT"
      exit 0
    fi
    echo "opm not running"
    exit 1
    ;;
  *)
