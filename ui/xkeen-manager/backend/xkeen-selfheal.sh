...
SING_BOX_INIT="/opt/etc/init.d/S24opm-singbox"
...
ensure_xray_init_confdir() {
  ...
  cp "$INIT" "$INIT.bak-opm-confdir" 2>/dev/null || true
...
repair_runtime() {
  # Master VPN off: keep it off. Tear down the redirect + stop daemons and skip
  # the keep-alive/restart logic, otherwise the watchdog would re-enable the VPN.
  if command -v xkeen_vpn_enabled >/dev/null 2>&1 && ! xkeen_vpn_enabled; then
    log "vpn disabled: teardown + stop daemons"
    xkeen_teardown_hooks 2>/dev/null || true
    xkeen_stop_xray 2>/dev/null || true
    xkeen_stop_singbox 2>/dev/null || true
    return 0
  fi
  log "repair start"
  ensure_xray_init_confdir
