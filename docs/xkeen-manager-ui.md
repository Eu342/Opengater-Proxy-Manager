# Runbook: Opengater UI

## Что это

`Opengater` — router-hosted UI для управления:
...
- `TCP` уходит в `61219`
- `UDP` идет напрямую везде, кроме доменов/CIDR в группах с outbound `vless-reality` — там TCP и UDP идут вместе через VPN
