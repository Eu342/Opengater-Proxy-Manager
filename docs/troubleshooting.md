...
Современные клиенты переходят с HTTPS на QUIC (UDP/443), как только сервер его поддерживает. По умолчанию Opengater не загоняет весь UDP политики в VPN — это ломает игры, RTC и IoT.
