...
`Opengater` — управляющий слой над `Keenetic + Entware + xray + sing-box`. Это router-hosted панель управления: и UI, и backend, и self-heal живут на самом роутере в `/opt/share/xkeen-manager/`.
...
Устройства, которым в Keenetic UI назначена политика с описанием `xkeen`, получают её mark. Mark **не хардкодится**: на разных роутерах он может быть разным (`0xffffaaa`, `0xffffaab`, ...). Opengater при каждой сборке runtime ищет mark динамически по описанию политики `xkeen`.

Инвариант безопасности: `Opengater` цепляется только к mark политики `xkeen`. Любые другие политики Keenetic (например, личная `no_vpn`) проектом не трогаются.
...
`xkeen-selfheal.sh` запускается каждые 15 секунд через watchdog `S25opengater-selfheal` и:
...
- cron-хук `cron.1min/50-opengater-selfheal` как страховочный слой;
- `ndm/fs.d/50-opengater.sh` и `ndm/usb.d/50-opengater.sh` — поднимают всё после возврата `/opt` или USB-событий;
- `S20opengater-sysctl` — точечно занижает TCP/conntrack-таймауты, чтобы fd на роутере не накапливались.
